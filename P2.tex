...
\hypersetup{
  ...
  pdfauthor={Du Shi, Kunning Zhang, Yixuan Yin},
  hidelinks,
...
alcohol concentration above the prescribed limit. When a person is
arrested for driving under the influence of alcohol it is not usually
possible to perform an accurate test of the level of alcohol in the
blood immediately. People are taken to a police station or hospital,
...
considering the heterogeneity between individuals, the model use
Hamiltonian Monte Carlo (one of Markov Chain Monte Carlo) to simulate
\(12000\) \(\beta\) draws, and gives a posterior distribution of
\(\hat{\beta}\) by using both samples and prior knowledge. From the
posterior distribution, we can find out the probability of the person's
BAC over limit while driving. The mean absolute error of results is only
\(0.023\).
...
explained by gender, female's \(\beta\) value is larger than male. But
we still find some useful variables like `weight' and `drinking time'.

We test the model on the given example, the 70 years-old woman drives
over the limit with \(91.1\) probability.

When it is too late to use a blood test and the only information
available is eyewitness testimony of the quantity of alcohol consumed.
We have to use Widmark's equation:
...
separately with \(\beta\). But \(V_d\) and \(\beta\) are not independent
(with correlation \(0.415\)), so the method is also unavailable. To
improve it, we build a joint Bayesian regression model, which can
simulate them together to solve with the correlation. \(V_d\) is
estimated by Total Body Water (TBW), we find an equation to calculate
it.
...
sample at position \(i+k\). Since Hamiltonian Monte Carlo is a random
walk process, \(\theta_i\) should be similar to \(\theta_{i+1}\), less
similar to \(\theta_{i+2}\), independent with \(\theta_{i+k}\) for large
\(k\). So if \(\rho_k\) is large means the chains hardly move, can't
converge.
...
peak and starts to decrease, so variables like `AAC' and `BACpeaktime'
will not be used, since they are correlated with the value of BAC, not
...
uses Hamiltonian Monte Carlo, which is better than standard MCMC (like
Metropolis-Hastings) since it is faster and cost less.
...
We have tried other variables, like we said in data description part,
some variables don't really explain much variation and some interactions
like sex with weight and height don't improve the model much, at least
with these \(100\) data. So we don't put them in.

`maxBAC' can improve the model a little bit, but in practice \(\beta\)
is a constant rate, `maxBAC' only matter for measured \(\beta\) since it
may affect the absorb time then affect the measured BAC elimination in
similar way of `drinking time', so we prefer to use `drinking time'
only.

\subsubsection{Priors}\label{priors}

Before fitting the Bayesian regression model, we should define
appropriate priors for all regression coefficients and the residual
standard deviation. We use weakly informative priors, which have small
effects on the posterior. The selected priors are summarized as follows:
...
or additional biological information, the prior distributions should be
changed.
...
intervals, \(98\) and \(51\) individuals' real \(\beta\) value is inside
the intervals. The MAE and RMSE are both relatively small.
...
\(12.993\)) means the validation results are good since we have \(10\)
parameters.
...
apprehended and the availability of a physician or nurse to draw blood.'
\citeproc{ref-jones2010evidence}{{[}4{]}}
...
variance of coefficients is set very small, the value of \(\beta\) is
mainly predicted by gender, which you can find clearly two groups for
prior B's model (figure \(12\)).
...
\section{Total words: 3764; Total executive summary words:
473}\label{total-words-3764-total-executive-summary-words-473}
...
\bibitem[\citeproctext]{ref-jones2010evidence}
\CSLLeftMargin{{[}4{]} }%
...
\emph{Forensic Science International}, vol. 200, no. 1--3, pp. 1--20,
2010, doi:
