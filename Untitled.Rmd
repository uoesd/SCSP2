...
We do two kinds of Cross-Validation: LOO and Kfold. The two methods show similar results means the model is stable and the $p$ value is near the number of variables ($5$) shows the model is not over-fitting.
...
Table $5$ belew is the results table, all Rhat and ESS are in good range. The expected estimated correlation $-0.46$ in the model is also near the sample's $V_d$-$\beta$ correlation $-0.415$ (the log-transformation of $\beta$ doesn't affect the correlation.)
...
Table $6$ is the LOO and Kfold results, the p values ($11.0479$ and $10.691$) means the validation results are good since we have $10$ parameters.
...
Table $7$ shows the MSE of $\beta$, $V_d$ and calculated $C_0$ from joint posterior distribution comparing with true value, and also a MSE for single $\beta$ model to test if the results worse when take in $V_d$. The estimation result for $\beta$ is better than before (because of the correlation, $V_d$ explain some variation of $\beta$).
