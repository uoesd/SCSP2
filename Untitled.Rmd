...
priors_table <- tribble(~Parameter, ~Prior,
                        ...
                        "Residual SD (σ)", "Exponential(1)")

kable(priors_table,
      caption = "Table 1: weak information priors used in the Bayesian regression model")
```

All variables have been standardized, so a normal distribution (0,1) is an appropriate weakly informative prior.
Future data considerations.

If future studies include different populations, larger sample sizes, or external biological knowledge (e.g., published estimates of sex differences in β), the priors can be adjusted—either narrowed (Normal(0,0.5)) when more information is available or widened (Normal(0,2)) when greater heterogeneity is expected. Prior-predictive checks should be used to verify suitability for any new dataset.
