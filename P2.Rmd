...
In this report, we introduce a Bayesian Regression Model (BRM) with considering the heterogeneity between individuals, the model use Hamiltonian Monte Carlo (one of Markov Chain Monte Carlo) to simulate $12000$ $\beta$ draws, and gives a posterior distribution of $\hat{\beta}$ by using both samples and prior knowledge. From the posterior distribution, we can find out the probability of the person's BAC over limit while driving. The mean absolute error of results is only $0.023$.

In real world, $\beta$ estimation is quite difficult, it depends on the individuals' liver condition, drinking habits, genetic, diet, etc. As we don't have corresponding data, variation of $\beta$ is mainly explained by gender, female's $\beta$ value is larger than male. But we still find some useful variables like 'weight' and 'drinking time'.

We test the model on the given example, the 70 years-old woman drives over the limit with $91.1$ probability.

When it is too late to use a blood test and the only information available is eyewitness testimony of the quantity of alcohol consumed. We have to use Widmark's equation:
...
Forensic scientists use the same method again to estimate $V_d$ separately with $\beta$. But $V_d$ and $\beta$ are not independent (with correlation $0.415$), so the method is also unavailable. To improve it, we build a joint Bayesian regression model, which can simulate them together to solve with the correlation. $V_d$ is estimated by Total Body Water (TBW), we find an equation to calculate it.
...
Since $\beta$ is a constant rate, measured when the BAC curve reaches peak and starts to decrease, so variables like 'AAC' and 'BACpeaktime' will not be used, since they are correlated with the value of BAC, not BAC elimination rate.
...
The $\theta$ in the equation is the $\gamma$s and $\varepsilon$. For most real models, we can't compute this posterior $p(\theta \mid \text{data})$ analytically. Markov Chain Monte Carlo (MCMC) helps us draw samples from it, which we can then summarize. BRM uses Hamiltonian Monte Carlo, which is better than standard MCMC (like Metropolis-Hastings) since it is faster and cost less.
...
- Gender: Female's BAC elimination rate is larger than male on average (figure 3), it is explained by liver weight represents a greater fraction of lean body mass in the female gender. @kwo1998gender
...
- Weight and Height: Basically higher weight often leads lager size of liver and larger amount of total body water. Height also affect the amount of total body water. @maskell2020total
...
We have tried other variables, like we said in data description part, some variables don't really explain much variation  and some interactions like sex with weight and height don't improve the model much, at least with these $100$ data. So we don't put them in. 

'maxBAC' can improve the model a little bit, but in practice $\beta$ is a constant rate, 'maxBAC' only matter for measured $\beta$ since it may affect the absorb time then affect the measured BAC elimination in similar way of 'drinking time', so we prefer to use 'drinking time' only.


### Priors

Before fitting the Bayesian regression model, we should define appropriate priors for all regression coefficients and the residual standard deviation. We use weakly informative priors, which have small effects on the posterior. The selected priors are summarized as follows:
...
If future research involves different populations, larger sample sizes, or additional biological information, the prior distributions should be changed.
...
```{r echo=FALSE, fig.width=10, fig.height=8, fig.cap="Posterioe density of 4 chains for all coefficients.", message=FALSE, warning=FALSE}
F4
```
...
```{r fig.width=10, fig.height=8, echo=FALSE, fig.cap="Traceplot of all coefficients and variance sigma.", message=FALSE, warning=FALSE}
F5
```
...
Table 3 shows the coverage rate of $95\%$ and $50\%$ predicted intervals, $98$ and $51$ individuals' real $\beta$ value is inside the intervals. The MAE and RMSE are both relatively small.
...
\begin{center}
There is $91.1\%$ probability that the 70 year old female is over the drink driving limit, with both mean, median and mode value over the limit.
\end{center}
...
Table $7$ is the LOO and Kfold results, the p values ($11.473$ and $12.993$) means the validation results are good since we have $10$ parameters.
...
Other than this, if we plot the mean posterior $\beta$ value with true $\beta$ value, the results will be more clear. Since the prior variance of coefficients is set very small, the value of $\beta$ is mainly predicted by gender, which you can find clearly two groups for prior B's model (figure $12$).
...
# Total words: 3764; Total executive summary words: 473
