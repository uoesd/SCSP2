...
\(\beta\) value have heavy tail. After simulation we can take
...
\(\beta\) as the the likelihood of the observed data. In our sample, it
is better to use the most common normal distribution likelihood, it is
...
the MCMC process of `sexfemale' when using \(4\) chains, \(50\) draws
with 30 warm-ups. Clearly that if we keep the first \(30\) draws, the
...
\subsection{Testing Model}\label{testing-model}
